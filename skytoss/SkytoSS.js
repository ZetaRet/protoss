...
function SkytoSS(){
	...
	m.argsTemplate=function(args){
		var obj={},i,l=args.length,v,vspl;
		for(i=0;i<l;i++){
			v=args[i];
			vspl=v.split(' ');
	...
	m.functionTemplate=function(name, returnType, args, props, main, prefix, suffix){
		var src,k,v,propArr=[],jl=o.joinLine,fh;
		if(props){
			for(k in props){
				v=props[k];
	...
	m.sourceTemplate=function(attr, mprops, main, functions, prepend, append){
		var src=[],k,attrArr=[],v,jl=o.joinLine;
	...
	m.lineWrapper=function(line, noreturn, props){
		var b=line.concat(),l=b.length-1,k,w;
	...
	m.gpuWrapRandomizer=function(seeder){
		var a={},f={},m=[];
		a.Seed={type:'highp float', avalue:seeder.Seed.toFixed(1), attrname:'varying'};
		a.rCount={type:'highp float', avalue:seeder.rCount.toFixed(1), attrname:'varying'};
		a.SEED_CONST={type:'highp float', value:seeder.SEED_CONST.toFixed(1), attrname:'const'};
		a.TIME_CONST={type:'highp float', value:seeder.TIME_CONST/1000, attrname:'const'};
		a.NUMBER_SEED_CONST={type:'highp float', value:seeder.NUMBER_SEED_CONST.toFixed(1), attrname:'const'};
		a.OVERFLOW_CONST={type:'highp float', value:seeder.OVERFLOW_CONST.toFixed(1), attrname:'const'};
		a.REAL_TIME={type:'highp float', value:seeder.REAL_TIME/1000, attrname:'const'};
		f.rGetTime={rtype:'highp float', main:o.lineWrapper(seeder.rGetTime)};
		f.rGetInt={rtype:'highp float', main:o.lineWrapper(seeder.rGetInt)};
		f.rGetNumber={rtype:'highp float', main:o.lineWrapper(seeder.rGetNumber)};
		f.rGetRange={rtype:'highp float', main:o.lineWrapper(seeder.rGetRange), args:['float rMin','float rMax']};
		f.rGetFloor={rtype:'highp float', main:o.lineWrapper(seeder.rGetMethod,false,{'rMethod':'floor'}), args:['float rMin','float rMax']};
		f.rGetCeil={rtype:'highp float', main:o.lineWrapper(seeder.rGetMethod,false,{'rMethod':'ceil'}), args:['float rMin','float rMax']};
		m.push('Seed='+seeder.Seed.toFixed(1)+';');
		m.push('rCount='+seeder.rCount.toFixed(1)+';');
		return {attr:a,code:o.sourceTemplate(a,null,null,f),main:m};
	};
	...
	m.useGPU=function(id, handler, settings, bufferData, readBufferType, glPreProgrammer, glProgrammer, glClear){
		...
		if(gpuData){
			if(gpuData.gpu && gpuData.program){
				var p,gl=gpuData.gpu,s=settings||{};
	...
	m.newWorker=function(src, id, type, options, srcurl){
		...
		if(!srcurl){
			b=new Blob([src],{type:'application/javascript'});
	...
	m.commandWorker=function(id, command, data, transfer){
		...
		if(w){
			var pm={command:command,target:o.skytossName,data:data};
	...
	m.deleteWorker=function(id){
		...
		if(w){
			...
			try{
			var pm={command:'terminate',target:o.skytossName};
...
SkytoSS.TEMPLATES={ITERATE:'iterate', IF_ELSE:'ifElse', ARGS:'args', FUNCTION:'function', SOURCE:'source', LINE:'line', SEED:'seed'};
SkytoSS.WORKERS={SHARED:'shared', NORMAL:'normal'};
...
SkytoSS.InitSkytoSSPrototypes=function(override){
	...
	oprot.const=function(key, value){
		Object.defineProperty(this, key, {
			...
			set: function(v){throw new TypeError('Assignment to constant variable.');}
		});
		return value;
	};
	odef(oprot, 'const', ef);
	...
	oprot.shell=function(keys){
		var l=keys.length,s={},i,t=this,k;
		for(i=0;i<l;i++){
			k=keys[i];
			s[k]=t[k].constructor;
		}
	...
	oprot.ys=function(shell, dome){
		if(!dome)dome=this;
		var s=dome.shell,h=dome.holes,k,sk,sc;
		for(k in h)
	...
	oprot.skydome=function(name, id, shell, holes, scope){
		...
		var dome={name:id||'Skydome',type:'skytoss_shell',shell:shell||{},holes:holes||{}};
		scope.internal(dome);
		return dome;
	};
