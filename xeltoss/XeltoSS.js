...
function XeltoSS() {
	...
	m.argumentKeyMatch = function(orshift, defval, formatter) {
		var f = function(obj, k, d, s) {
			var kv = "",
				...
				akl = ak.length,
				lk = k.toLowerCase(),
				i, aki, augo = o.augmentKeyMap[s],
				nkic = o.noKeyIdentificationChain[s];
			...
			if (defval !== undefined) {
				if (defval === "self") {
					...
					kv += (kv ? "||" : "") + "new " + (ok.constructor.packagename ? ok.constructor.packagename + "." : "") + (ok.constructor.aname || ok.constructor.name) + "(" + o.getConstructorArgs(
						ok.constructor, ok, k) + ")";
				} else if (defval === "void") {
					return undefined;
	...
	m.toCls = function(obj, clsname, clssuper, deflat, polymaps, reservedwordsmap, emptify) {
		...
		var cls = '',
			...
			str, stri = -1,
			em = o.embedMaps,
			emv, rwm = reservedwordsmap,
			clsh = o.classHandler,
			oeh = o.overextendHandler,
			alset = o.allowSetters,
			alget = o.allowGetters,
			alac = o.allowAsync,
			algen = o.allowGenerator,
			ba = o.bodyAssembler,
			ma = o.methodAssembler,
			ckeys = o.constructorKeys;
		...
		var supers = obj.getSupers(),
			rsupers = obj.getReversedSupers(),
			mapsupers = [obj.constructor].concat(supers),
			mapnames = [],
			mymethods = obj['__' + clsname + '_super__'],
			sname = obj.getSuperName2(),
			_sname, tp = o.toppack,
			keyedoutmaps = [],
			iamethods = [],
			inherited = [],
		...
		for (k in obj) {
			...
			if (em[sname] && em[sname].hasOwnProperty(k)) {
				...
				else clsb.push((rwm.t || 'this') + '.' + k + '=' + (rwm.nw || 'new') + ' ' + (ok.constructor.packagename ? ok.constructor.packagename + "." : "") + (ok.constructor.aname ||
					ok.constructor.name) + '(' + o.getConstructorArgs(ok.constructor, obj, k) + ');');
				if (ba) ba.call(obj, k, ok, o, clsname, clssuper, clsArgs, superArgs, clsb, clsf, polymaps, rwm);
			} else if (oeh) {
	...
	m.toClsBlobScript = function(clss, prefix, suffix) {
		if (!prefix) prefix = "";
		if (!suffix) suffix = "";
		var b = new Blob([prefix + clss + suffix], {
	...
	m.reconstruct = function(cls, clsname, clssuper, deflat, polymaps, reservedwordsmap, emptify, useclsfactory) {
		var n = cls.aname || cls.name,
			po = cls.packobj || window,
			obj, newcls, clss;
	...
	m.hybrid = function(escls, protcls, obj, swap, polymaps) {
		var n = protcls.aname || protcls.name,
			po = protcls.packobj || window,
			promap = obj["__" + n + "_super__"],
			k, i, ss, xs, sn,
			...
			supers = obj.getSupers(),
			supers2 = obj.getSupers2();
						...
						if (o.methodTransfer) {
							for (k in promap) {
								if (k !== "constructor" && !newclsproto[k]) {
									...
									Object.defineProperty(newclsproto, k, {
										enumerable: false
									});
								}
							}
	...
	m.protoss = function(cls) {
		var n = cls.aname || cls.name,
			po = cls.packobj || window,
			protosscls = po[o.protossPrefix + n];
		po[n] = protosscls;
		return protosscls;
	};
...
XeltoSS.InitXeltoSSPrototypes = function(override) {
	...
	Function.prototype.xcoped = function(scope) {
		var superf = this;
		var scopedf = function scopedf() {
			return superf.apply(scope, arguments);
		};
		scopedf.xf = superf;
		scopedf.xcope = scope;
