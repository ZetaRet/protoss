...
function ZetaRet_Prototypes() {
	...
	oprot.getNextSuperX = function(name, cname, thiscls) {
		...
		var l = _s.length,
			tfmn = thiscls ? prfx + (thiscls.aname || thiscls.name) + sffx : null,
			tf = (thiscls ? ((thiscls[prn][tfmn] ? thiscls[prn][tfmn][name] : null) || this[tfmn][name]) : this[name]);
		for (var i = 0; i < l; i++) {
			var si = _s[i];
			var m = this[prfx + (si.aname || si.name) + sffx];
			var f = (m ? m[name] : null);
					...
					try {
						if (cls.constructor === Array) {
							for (j = 0; j < cls.length; j++) {
								if (ai.is(cls[j])) break;
							}
							if (j === cls.length) return false;
						} else {
							if (!ai.is(cls)) return false;
						}
					} catch (Object) {
						return false;
					}
